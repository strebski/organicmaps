...
public final class Config
{
   ...
   */
  private static final String KEY_APP_LAST_LAUNCH_TIMESTAMP = "LastLaunchTimestamp";
  /**
   * The overall count of sessions, where one session = one calendar day.
   */
  private static final String KEY_APP_SESSION_NUMBER = "SessionNumber";
  /**
   * The start timestamp of the session, where one session = one calendar day.
   */
  ...
  /**
   * The flavor of first instaklled version of the app.
   */
  ...
  /**
   * The flavor of the last launched version of the app.
   */
  ...
  public static void updateCounters(@NonNull Context context)
  {
    ...
    editor.putLong(KEY_APP_LAST_LAUNCH_TIMESTAMP, System.currentTimeMillis());
    ...
    final long lastSessionTimestamp = prefs.getLong(KEY_APP_LAST_SESSION_TIMESTAMP, 0);
    if (!DateUtils.isToday(lastSessionTimestamp))
    {
      final int sessionNumber = prefs.getInt(KEY_APP_SESSION_NUMBER, 0);
